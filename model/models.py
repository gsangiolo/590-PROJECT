...
def Inception_Img(img_size, num_classes):
...
def MobileNet(img_size, num_classes):
    ...
    model.add(MobileNetV2(include_top=False, input_shape=(img_size, img_size, 3), weights='imagenet'))
    model.add(Flatten())
    model.add(Dense(512))
    model.add(Activation('relu'))
    model.add(Dropout(0.1))
...
def ResNet50_Img(img_size, num_classes):
